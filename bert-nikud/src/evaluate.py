...
def evaluate(
    model, dataloader: DataLoader, device: str, tokenizer, desc: str = "Evaluating"
) -> Dict[str, float]:
    """
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc, leave=False):
            ...
            vowel_correct += (
                (vowel_preds[vowel_mask] == vowel_labels[vowel_mask]).sum().item()
            )
            vowel_total += vowel_mask.sum().item()

            dagesh_mask = dagesh_labels != -100
            dagesh_correct += (
                (dagesh_preds[dagesh_mask] == dagesh_labels[dagesh_mask]).sum().item()
            )
            ...
            stress_correct += (
                (stress_preds[stress_mask] == stress_labels[stress_mask]).sum().item()
            )
            stress_total += stress_mask.sum().item()

            prefix_mask = prefix_labels != -100
            prefix_correct += (
                (prefix_preds[prefix_mask] == prefix_labels[prefix_mask]).sum().item()
            )
